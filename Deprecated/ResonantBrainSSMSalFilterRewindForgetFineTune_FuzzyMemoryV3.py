...
# Standard ChatML tokens, registered as explicit special tokens so tiktoken
# does not raise an error when encoding them as plain text.
CHAT_START = "<|im_start|>"
CHAT_END = "<|im_end|>"
...
class TiktokenTokenizer:
    def __init__(self, encoding_name="gpt2"):
        ...
        # Explicitly register ChatML special tokens so tiktoken treats them as
        # single atomic tokens rather than splitting them into characters.
        special_tokens = {
            "<|im_start|>": base_tokenizer.n_vocab,
            "<|im_end|>": base_tokenizer.n_vocab + 1
        }
    ...
    def encode(self, text):
        return self.tokenizer.encode(text, allowed_special={"<|im_start|>", "<|im_end|>", "<|endoftext|>"})
...
def get_forgetting_config(layer_idx, num_layers, enable_forgetting):
    ...
    return {
        ...
        'decay_factor': 0.95 + (depth_ratio * 0.018),      
...
def token_generator_from_parquet(files, text_column, tokenizer):
    """Legacy generator without progress tracking"""
    for file in files:
        try:
...
class ParquetProgressTracker:
    """Tracks progress through parquet files during training"""
    def __init__(self, files, text_column, tokenizer, dataset_name="default", log_file=None):
        self.files = files
        self.text_column = text_column
        self.tokenizer = tokenizer
        self.dataset_name = dataset_name
        self.log_file = log_file or f'trained_parquet_files_{dataset_name}.log'
        
        # Progress tracking
        self.current_file_idx = 0
        self.current_file = None
        self.current_file_tokens_yielded = 0
        self.current_file_total_tokens = 0
        
        # Load already processed files
        self.completed_files = self._load_completed_files()
        
        # Filter out already completed files
        self.files = [f for f in self.files if f not in self.completed_files]
        
        if self.completed_files:
            print(f"[INFO] Skipping {len(self.completed_files)} already trained parquet files")
        if not self.files:
            print(f"[INFO] All parquet files have been trained on!")
    
    def _load_completed_files(self):
        """Load list of already completed parquet files"""
            ...
            with open(self.log_file, 'r', encoding='utf-8') as f:
                for line in f:
                    ...
                    if line and not line.startswith('#'):
                        completed.add(line)
            print(f"[INFO] Loaded {len(completed)} completed files from {self.log_file}")
        except Exception as e:
            print(f"[WARNING] Failed to load log file {self.log_file}: {e}")
        
        return completed
    
    def _log_completed_file(self, file_path):
        """Append completed file to log"""
        try:
            with open(self.log_file, 'a', encoding='utf-8') as f:
                f.write(f"{file_path}\n")
                f.flush()  # Ensure immediate write
        except Exception as e:
            print(f"[WARNING] Failed to log completed file {file_path}: {e}")
    
    def _count_tokens_in_file(self, file_path):
        """Count total tokens in a parquet file (for progress calculation)"""
        try:
            parquet_file = pq.ParquetFile(file_path)
            total_tokens = 0
            for batch in parquet_file.iter_batches(batch_size=500, columns=[self.text_column]):
                df = batch.to_pandas()
                for text in df[self.text_column].dropna():
                    total_tokens += len(self.tokenizer.encode(str(text)))
            return total_tokens
        except Exception as e:
            print(f"[WARNING] Failed to count tokens in {file_path}: {e}")
            return 0
    
    def generate_tokens(self):
        """Generator that yields tokens and tracks progress"""
        for file_idx, file in enumerate(self.files):
            self.current_file_idx = file_idx
            self.current_file = file
            self.current_file_tokens_yielded = 0
            
            # Count total tokens in file for progress tracking
            print(f"\n[INFO] Counting tokens in {os.path.basename(file)}...")
            self.current_file_total_tokens = self._count_tokens_in_file(file)
            print(f"[INFO] File contains ~{self.current_file_total_tokens:,} tokens")
            ...
            try:
                ...
                for batch in parquet_file.iter_batches(batch_size=500, columns=[self.text_column]):
                    ...
                    for text in df[self.text_column].dropna():
                        ...
                        self.current_file_tokens_yielded += len(tokens)
                        yield from tokens
                
                # Mark file as completed
                self._log_completed_file(file)
                print(f"\n[INFO] ✓ Completed parquet file: {os.path.basename(file)}")
    ...
    def get_progress_info(self):
        ...
        progress_pct = 0.0
        if self.current_file_total_tokens > 0:
            progress_pct = (self.current_file_tokens_yielded / self.current_file_total_tokens) * 100
        
        return {
            'file_name': os.path.basename(self.current_file),
            'file_idx': self.current_file_idx + 1,
            'total_files': len(self.files),
            'progress_pct': progress_pct,
            'tokens_processed': self.current_file_tokens_yielded,
            'total_tokens': self.current_file_total_tokens
        }

class WeightedDatasetStream:
    """Manages sampling from multiple datasets according to configured weights"""
    def __init__(self, dataset_config, text_column, tokenizer):
        ...
        for name, config in dataset_config.items():
            ...
            if not files:
                print(f"[WARNING] No parquet files found for dataset {name} at {path}")
                continue
                
            random.shuffle(files)
            tracker = ParquetProgressTracker(files, text_column, tokenizer, dataset_name=name)
            
            self.trackers[name] = tracker
            self.generators[name] = tracker.generate_tokens()
            self.dataset_names.append(name)
            self.weights.append(weight)
        
        if not self.dataset_names:
            raise RuntimeError("No valid datasets found to stream from!")
            
        print(f"[INFO] Initialized {len(self.dataset_names)} active dataset streams")
        
    def generate_tokens(self, chunk_len=512, domain_switch_interval=4):
        """Yields chunks of tokens sampled according to dataset weights"""
        while True:
            # Pick a dataset based on weights
            sampled_dataset = random.choices(self.dataset_names, weights=self.weights, k=1)[0]
            gen = self.generators[sampled_dataset]
            
            # We want to yield a substantial continuous chunk to ensure the model doesn't overfit
            # on the context boundary reset.
            # We'll fetch enough for multiple full context blocks to allow extended sequences
            num_tokens_to_fetch = chunk_len * domain_switch_interval
            
            chunk = []
            try:
                for _ in range(num_tokens_to_fetch):
                    chunk.append(next(gen))
                    
                for token in chunk:
                    yield token, sampled_dataset
                    
            except StopIteration:
                print(f"[INFO] Dataset '{sampled_dataset}' exhausted.")
                # Remove this dataset and its weight
                idx = self.dataset_names.index(sampled_dataset)
                self.dataset_names.pop(idx)
                self.weights.pop(idx)
                
                # yield whatever we managed to grab before exhaustion
                for token in chunk:
                    yield token, sampled_dataset
    ...
    def get_progress_info(self, dataset_name):
        return self.trackers[dataset_name].get_progress_info()
...
def print_gate_stats(model, iteration, running_loss, train_steps, scheduler, step_type="CLEAR", progress_info=None):
    current_lr = scheduler.get_last_lr()[0]
    log_str = f"[Step {iteration}] Type: {step_type:5s} | Loss: {running_loss / max(1, train_steps):.4f} | LR: {current_lr:.2e}"
    
    # Add parquet progress information if available
    if progress_info:
        log_str += f" | File: {progress_info['file_name']} ({progress_info['file_idx']}/{progress_info['total_files']}) | Progress: {progress_info['progress_pct']:.1f}%"
...
def run_pretraining(model, dataset_config, text_column, tokenizer, optimizer, device,
    ...
    # Cosine LR decay: decays from current LR down to lr_min over lr_decay_steps total steps
    _initial_lr = optimizer.param_groups[0]['lr']
    _min_ratio = lr_min / max(_initial_lr, 1e-12)
    def _cosine_lr_lambda(step):
        global_step = start_iteration + step
        progress = min(1.0, global_step / lr_decay_steps)
        cosine = 0.5 * (1.0 + math.cos(math.pi * progress))
        return max(_min_ratio, cosine)
    scheduler = torch.optim.lr_scheduler.LambdaLR(optimizer, _cosine_lr_lambda)
    print(f"--- LR Schedule: cosine decay {_initial_lr:.2e} → {lr_min:.2e} over {lr_decay_steps} steps (currently at step {start_iteration}) ---")
    ...
    # Use weighted dataset streams for parquet files
    stream_manager = WeightedDatasetStream(dataset_config, text_column, tokenizer)
...
def chat_mode(model, tokenizer, device, chunk_size=512):
    ...
    # Keep model in full precision for inference stability
    # Mixed precision is only used during training with autocast
    model.eval()
...
def chat_mode_full_agi(agi_systems, tokenizer, device, chunk_size=512):
    ...
    # Keep model in full precision for inference stability
    # Mixed precision is only used during training with autocast
    print(f"[DEBUG] Model ready for inference (full precision)")
    thinking_model.eval()
...
if __name__ == "__main__":
    ...
    DATASET_CONFIG = {
        "FineWeb":      {"path": r"I:\Datasets\FineWeb",   "weight": 0.60},
    ...
    if choice == '1':
        ...
        if os.path.exists(ckpt_path) and input("Resume pre-training checkpoint? (y/n): ").strip().lower() == 'y':
            ...
            # Force LR to current LEARNING_RATE — load_state_dict restores the old saved LR
            for param_group in optimizer.param_groups:
                param_group['lr'] = LEARNING_RATE
            start_it = ckpt.get('iteration', 0)
            print(f"[INFO] Loaded checkpoint (filtered mismatched keys). LR reset to {LEARNING_RATE:.2e}")
            
        run_pretraining(
            training_model, DATASET_CONFIG, "text", tokenizer, optimizer, device, vocab_size, start_it, 
            CHUNK_SIZE, ENABLE_COGNITIVE_FORGETTING, BATCH_SIZE, GRAD_ACCUM_STEPS,
            FUZZY_STEPS, CLEAR_STEPS, BAG_SIZE, THINKING_LOSS_WEIGHT, DOMAIN_SWITCH_INTERVAL,
            lr_decay_steps=LR_DECAY_STEPS, lr_min=LR_MIN
        )
